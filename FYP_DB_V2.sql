...
create table DistributorProductDetail(
	id int not null identity(1,1) primary key,
	no_of_cartons int,
	batch_no int ,--foreign key references batch_production(batch_no),
...
CREATE TRIGGER update_num_cartons ON batch_production
AFTER update
AS
BEGIN
DECLARE @id INT
...
--create trigger add_batchIN_OrderedItems on OrderDetail
--after delete
--as
--begin
--DECLARE @oid INT
--set @oid =  (select order_id from deleted)
--DECLARE @pid INT
--set @pid =  (select product_id from deleted)
--DECLARE @qty INT
--set @qty =  (select qty_ordred from deleted)

--end

--create proc updateOrderedProducts( @oid int, @qty int, @pid int) as
--begin
--end
